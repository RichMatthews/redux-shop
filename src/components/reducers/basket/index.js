...
const increaseItemQuantity = (items, action) => {
  return items.map((product) => {
...
const decreaseItemQuantitiy = (items, action) => {
  return items.map((product) => {
    if (product.product.id == action.data.product.product.id) {
...
export default (state = initialState, action) => {
  switch(action.type) {
    ...
    case "REMOVE_FROM_BASKET":
      return { items: removeProductFromBasket(state.items, action), total: calculateTotal(state.items)}
    case "INCREASE_QUANTITY":
      return { items: increaseItemQuantity(state.items, action), total: calculateTotal(state.items)}
    case "DECREASE_QUANTITY":
      return { items: decreaseItemQuantitiy(state.items, action), total: calculateTotal(state.items)}
    case "CLEAR_BASKET":
